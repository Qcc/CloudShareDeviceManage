...
<div>
  ...
  <el-form :model="createForm" >
    <el-form-item v-for="item in createForm" v-bind:key="item.key" v-bind:label="item.label">
      <el-date-picker v-if="item.type === 'DATE'" v-model="createForm.value" type="datetime" placeholder="选择日期时间"></el-date-picker>
    </el-form-item>
  </el-form>
  </el-dialog>
</div>
  ...
  methods: {
    ...
    handleCreateCOl () {
      ...
      for (let i = 0; i < this.tableCol.length; i++) {
        ...
        this.createForm[i] = { key: key, label: label, type: type, value: '' }
      }
    }
  }
}
